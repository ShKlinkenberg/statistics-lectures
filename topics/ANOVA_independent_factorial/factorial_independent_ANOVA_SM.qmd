...
$b_0 + b_1 \times \text{some alcohol} + b_2 \times \text{much alcohol} + b_2 \times \text{weight}$

## Aalcohol x Weight

$$
\begin{aligned}
b_0 & + b_1 \times \text{some alcohol} \\
    & + b_2 \times \text{much alcohol} \\
    & + b_3 \times \text{weight} \\
    & + b_4 \times \text{some alcohol} \times \text{weight} \\
    & + b_5 \times \text{much alcohol} \times \text{weight} \\
\end{aligned}
$$
...
$$
\begin{aligned}
`r b_0` & + `r b_1` \times \text{some}_{01} \\
        & + `r b_2` \times \text{much}_{01} \\
        & + `r b_3` \times \text{weight} \\
        & + `r b_4` \times \text{some}_{01} \times \text{weight} \\
        & + `r b_5` \times \text{much}_{01} \times \text{weight} \\
\end{aligned}
$$

## Data with dummies {.smaller}

$\tiny `r b_0` + `r b_1` \times \text{some}_{01}
         + `r b_2` \times \text{much}_{01}
         + `r b_3` \times \text{weight}
         + `r b_4` \times \text{some}_{01} \times \text{weight}
         + `r b_5` \times \text{much}_{01} \times \text{weight}$
         
<small style="transform: scale(0.8); margin:0; padding:0;">

```{r}
some = ifelse(data$alcohol == "some", 1, 0)
much = ifelse(data$alcohol == "much", 1, 0)

weight = data$weight

reg.data <- data.frame(b_0, b_1, some, b_2, much, b_3, weight, b_4, some, much, b_5, some, much, weight)

datatable(reg.data, options = list(searching = FALSE, scrollY = 415, paging = F, info = F))
```
