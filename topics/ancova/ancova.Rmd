...
## Model fit with covar {.smaller}

```{r}
fit <- lm(outcome ~ factor(group) + covar, data)
fit
...
```{r}
plot + geom_segment(aes(x = n-.1, y = outcome, xend = n-.1, yend = data$model, colour = "purple")) -> plot
plot
```
...
SS.model - SS.model.group ## SS.covar corrected for group
SS.model - SS.model.covar ## SS.group corrected for covar
```
