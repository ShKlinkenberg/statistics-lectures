...
![Beasts](../../../topics/bias/here-be-dragons.jpg)
...
IQ estimations of males and females. We want to know the differences in the population not the sample. We therefore want to make an inference about the population, hence the name inferential statistics. 
...
$$\text{IQ you}_i = b_0 + b_1 males_i + error_i$$

```{r, echo=FALSE}
means <- aggregate(IQ.you ~ factor(gender), data, mean)
```

```{r}
aggregate(IQ.you ~ factor(gender), data, mean)
```

We can now calculate the $b$'s: $b_0 = `r means[1,2]`$ and $b_1 = `r means[2,2] - means[1,2]`$
...
```{r, echo=FALSE}
fit <- lm(IQ.you ~ factor(gender), data)
cbind(IQ.you = data$IQ.you, b.0 =fit$coefficients[1], b.1 = fit$coefficients[2], gender = data$gender, error = fit$residuals)[12:17,]
```
...
But what if these means are not correct, becouse of an extreme outlier.

```{r, echo=FALSE}
boxplot(data$IQ.you, horizontal = T, col= 'red', main=names(data)[3])
```

## Outliers {.subsection}

```{r, echo=FALSE, fig.height=2.2, eval=FALSE}
...
aggregate(IQ.you ~ factor(gender), data2, mean)

fit <- lm(IQ.you ~ factor(gender), data2)
cbind(IQ.you = data2$IQ.you, b.0 =fit$coefficients[1], b.1 = fit$coefficients[2], gender = data2$gender, error = fit$residuals)[12:17,]
```
