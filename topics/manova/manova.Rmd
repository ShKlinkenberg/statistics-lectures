...
Compare multiple independent variables on multiple dependent variables.
...
Multivariate analysis of variance. We are looking for the multivariate signal to noise ratio.

## Example {.smaller}

We will use the data from Field's chapter 16 about behavioural and cognitive neurosis. Let's first load the data and have a look.
...
```{r}
cbt.tho = subset(data, group == "cognitieve gedrags therapie", select="thoughts")$thoughts
bet.tho = subset(data, group == "gedrags therapie",            select="thoughts")$thoughts
con.tho = subset(data, group == "Controle groep",              select="thoughts")$thoughts

cbt.act = subset(data, group == "cognitieve gedrags therapie", select="actions")$actions
bet.act = subset(data, group == "gedrags therapie",            select="actions")$actions
con.act = subset(data, group == "Controle groep",              select="actions")$actions
...
## Multivariate signal to noise {.smaller}
...
## Pillai-Barrtlett trace

> Pillai’s trace is the sum of the proportion of explained variance on the discriminant functions.

Excerpt From: Field, Andy. “Discovering Statistics Using IBM SPSS Statistics 4th c2013 Andy Field.”. 
